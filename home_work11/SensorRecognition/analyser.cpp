...
void Analyser::analyseData(const SensorMetric& aSensorMetric){
    dataSensorsMetric[aSensorMetric.name].append(aSensorMetric);
}

void Analyser::addSensor(QString name)
{
    int number = 1;
    while(dataSensorsMetric.contains(QString(name).arg(QString::number(number)))){
        ++number;
    }
    dataSensorsMetric[QString(name).arg(QString::number(number))].append(*new QList<SensorMetric>());
}

void Analyser::deleteSensor(QString name)
{
    if(dataSensorsMetric.contains(name)){
        dataSensorsMetric.remove(name);
    }
    emit sendSensorsData(&dataSensorsMetric);
}

void Analyser::simulateSensor()
{

}

void Analyser::slotSensorData()
{
    emit sendSensorsData(&dataSensorsMetric);
}
